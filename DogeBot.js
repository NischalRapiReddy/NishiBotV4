...
module.exports = DogeXeonOP = async (DogeXeonOP, mek, _welkom) => {
	try {
		...
		const ownerNumber = [`${owner}@s.whatsapp.net`, `918919822103@s.whatsapp.net`]
...
const textImg = (teks) => { return DogeXeonOP.sendMessage(from, teks, text, {quoted: fgi,})}
// thumbnail: fs.readFileSync('./media/dogetb.jpg')
const fakeitem = (teks) => { DogeXeonOP.sendMessage(from, teks, text, { quoted: { key:{ fromMe:false, participant:`0@s.whatsapp.net`, ...(from ? { remoteJid :"6289523258649-1604595598@g.us" }: {})},message:{"orderMessage":{"orderId":"174238614569481","itemCount":9999999999,"status":"INQUIRY","surface":"CATALOG","message": `${ucapanWaktu} ${pushname}`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}},contextInfo: {"forwardingScore":999,"isForwarded":true}, sendEphemeral: true})}
// "thumbnail":fs.readFileSync("./media/dogetb.jpg")
...
╰───「 *🥳ʟᴇᴠᴇʟ ᴜᴘ🥳* 」`)
          ...
          sendButton(from, lvlup, '*Nishi Bot*', but)
...
${gaya1} *Creator Name* : ${ownername}
${gaya1} *Creator Number* : @${pemilik.split('@')[0]}
...
DogeXeonOP.sendMessage(from, { contentText: `${teks}`, footerText: `${menu}`, buttons: [{ buttonId: `${prefix}command`, buttonText: { displayText: 'MENU 🗃️' }, type: 1 },{ buttonId: `${prefix}sc`, buttonText: { displayText: 'Gallz Dm Me;) 📝' }, type: 1 },{ buttonId: `${prefix}developer`, buttonText: { displayText: 'My Master 👨🏼‍💻' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: fakeimage, contextInfo: {mentionedJid: [senderr,pemilik]}}}, 'buttonsMessage')
break
case 'command':
...
Please Choose Whether Document, Audio or Video Below`
        sendButMessage(from, result, `*Nishi Bot*`, [
          {
...
sendButVideo(from, pll, `*Nishi Bot*`, get_result, [      
{
buttonId: `${prefix+command}`,
buttonText: {
displayText: `NEXT ➡️`,
...
+ 'VERSION:3.0\n'
+ `FN:${developerName}\n`
+ `ORG: Developer Of Nishi Bot ;\n`
+ `TEL;type=CELL;type=VOICE;waid=${developerNo}:${developerNo}\n`
+ 'END:VCARD'.trim()
DogeXeonOP.sendMessage(from, {displayName: `The creator ${botname}`, vcard: vcard2}, contact, 
{ quoted: fgi, 
})
...
{ buttonId: `${prefix}developer`, buttonText: { displayText: 'DM MY MASTER 👨🏼‍💻' }, type: 1 }
]
...
case 'memegenerator': case 'memegen':{
 	...
 	case 'stickermeme': case 'memesticker': case 'memestick': case 'stickmeme': case 'stcmeme': case 'smeme':{
						if (args.length < 1) return reply(`Send orders *${prefix + command}* NishiBot`)
...
case 'bc':
             ...
             for (let _ of anu100) {
             DogeXeonOP.sendMessage(_.jid, bc100, image, {quoted: fgi, caption: `*「 Nishi Bot Broadcast 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu100) {
             DogeXeonOP.sendMessage(_.jid, 
			{"contentText": `*「 Nishi Bot BROADCASTING MESSAGE 」*\n\n${body.slice(4)}`,
			"footerText": `${tanggal}`,
			"buttons": [
...
DogeXeonOP.sendMessage(_.jid, bcgc, image, { caption: `*「 Nishi Bot BROADCAST 」*\n*Group* : ${groupName}\n\n${body.slice(6)}` })
}
reply('')
} else {
for (let _ of groupMembers) {
sendMess(_.jid, `*「 Nishi Bot BROADCAST 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
}
...
reply(`${err}`)
	...
	DogeXeonOP.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "D0D0 AKA Nischal",body:"Do You Love me?",previewType:"PHOTO",sourceUrl:"https://wa.me/918919822103"}}})
